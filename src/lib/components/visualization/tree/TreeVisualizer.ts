...
export class TreeVisualizer {
  ...
  public highlightNode(nodeId: string | null) {
    ...
    this.g.selectAll('g.node').classed('is-selected', (d: unknown) => (d as PointNode).data.id === nodeId);
  }

  public setReadingNode(nodeId: string | null) {
    if (!this.g || !this.d3) return;
    
    this.g.selectAll('g.node').classed('is-reading', (d: unknown) => (d as PointNode).data.id === nodeId);
  }
  ...
  public highlightPathToNode(nodeId: string | null) {
    ...
    if (!nodeId) {
      this.g.selectAll('g.node').classed('is-dimmed is-ancestor is-descendant', false);
      this.g.selectAll('path.link').classed('is-dimmed is-ancestor is-descendant', false);
      return;
    }
    ...
    this.g.selectAll<SVGGElement, PointNode>('g.node')
    ...
    this.g.selectAll('path.link')
      .filter((l: unknown) => {
          ...
          return ancestorIds.has(link.source.data.id) && ancestorIds.has(link.target.data.id);
      })
      .classed('is-dimmed', false)
      .classed('is-ancestor', true);
    ...
    this.g.selectAll('path.link')
      .filter((l: unknown) => {
          const link = l as { source: PointNode; target: PointNode };
          return descendantIds.has(link.source.data.id) && descendantIds.has(link.target.data.id);
      })
      .classed('is-dimmed', false)
      .classed('is-descendant', true);
  }
  ...
  private setupSVG() {
    ...
    // Zoom
    this.zoomBehavior = this.d3.zoom<SVGSVGElement, unknown>()
      .scaleExtent([0.2, 2])
      .on('zoom', (event) => {
  ...
  private update(source: PointNode) {
    ...
    const transition = this.d3.select(this.g.node()!).transition().duration(this.transitionDuration);

    // Nodes
    const node = this.g.selectAll<SVGGElement, PointNode>('g.node')
    ...
    // Links
    const link = this.g.selectAll<SVGPathElement, Link<unknown, PointNode, PointNode>>('path.link')
    ...
    link.merge(linkEnter).transition(transition as any).attr('d', (d) => this.linkPath(d as any));
  ...
  private updateNodeContent(nodeUpdate: any) {
    ...
    nodeUpdate.select('.indicator')
      ...
      .attr('transform', `translate(${this.nodeWidth - 10}, ${-this.nodeHeight / 2 + 10})`);
  }
  ...
  private expandPathToNode(targetNode: PointNode) {
    ...
    targetNode.ancestors().forEach((ancestor) => {
      if (ancestor.data.id !== targetNode.data.id && !this.expandedNodeIds.has(ancestor.data.id)) {
