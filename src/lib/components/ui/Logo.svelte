<!--
  ...
  Responsibilities:
  - Renders a clean, readable SVG of the logo.
  - Accepts a `size` prop to dynamically control its width and height.
  - Uses `fill="currentColor"` for effortless styling via the parent's CSS `color` property.
  - Includes a subtle, polished hover animation for a delightful micro-interaction.

  Props:
  - `size`: The width and height of the logo in pixels. Defaults to 24.
-->
<script lang="ts">
  /** ENHANCEMENT: Abstracting the complex SVG path data into a constant makes the template clean and readable. */
  ...
  /** ENHANCEMENT: Modernized to use Svelte 5 runes. */
  let {
    /**
     * @prop {number} [size=24]
...
<svg
  ...
  class="app-logo"
>
  <path fill="currentColor" d={LOGO_PATH_DATA} />
</svg>

<style>
  .app-logo {
    color: inherit;
    /** ENHANCEMENT: Add a smooth transition for a polished hover effect. */
    transition: transform 0.3s cubic-bezier(0.5, 1.5, 0.5, 1);
  }

  /* The parent element can control the hover effect */
  :global(a:hover) .app-logo,
  :global(button:hover) .app-logo {
    /** ENHANCEMENT: A subtle rotation adds a delightful touch of personality. */
