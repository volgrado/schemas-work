<!--
  src/lib/modules/visualization/ui/StandardTree.svelte
...
<script lang="ts">
  ...
  import type { TreeNodeData } from '../domain/types';

  // Type-only imports for D3
  ...
  let rootNode = $state<HierarchyNode<TreeNodeData> | null>(null);
  ...
  // D3 Module State
  let d3 = $state<any>(null);
  let isD3Loaded = $state(false);

  onMount(async () => {
    const [d3Selection, d3Hierarchy, d3Zoom, d3Shape, d3Transition] =
      await Promise.all([
        import('d3-selection'),
        import('d3-hierarchy'),
        import('d3-zoom'),
    ...
    d3 = {
      ...
      ...d3Zoom,
      ...d3Shape,
      // transition is side-effect, but we might need symbols if exported
    };
    isD3Loaded = true;
  });
  ...
  type TreeLayoutResult = HierarchyPointNode<TreeNodeData>;
  ...
  $effect(() => {
    ...
    const root = d3.hierarchy(treeData) as HierarchyNode<TreeNodeData>;
  ...
  $effect(() => {
    if (!rootNode || !gEl || !d3) {
      if (gEl && d3) d3.select(gEl).selectAll('*').remove();
      return;
    }
    tick().then(() => {
  ...
  $effect(() => {
    ...
    d3.select(gEl)
      .selectAll('g.node')
      .classed('is-selected', (d: PointNode) => d.data.id === selectedNodeId);
  });
  ...
  $effect(() => {
    ...
    d3.select(gEl)
      .selectAll('g.node')
      .classed('is-reading', (d: PointNode) => d.data.id === activeNodeId);
  });
  ...
  function panToNode(targetNode: PointNode) {
    if (!svgEl || !zoomBehavior || !d3) return;
    const svg = d3.select(svgEl);
    const { width, height } = svg.node()!.getBoundingClientRect();
    // Get current scale from the transform
    const svgNode = svg.node()!;
    const currentTransform = (svgNode as any).__zoom || d3.zoomIdentity;
  ...
  function highlightNodePath(targetNode: PointNode | null) {
    ...
    g.selectAll('g.node')
      .filter((n: PointNode) => ancestorIds.has(n.data.id))
      .classed('is-dimmed', false)
      .classed('is-ancestor', true);

    g.selectAll('g.node')
      .filter((n: PointNode) => descendantIds.has(n.data.id))
      .classed('is-dimmed', false)
      .classed('is-descendant', true);

    g.selectAll('g.node')
      .filter((n: PointNode) => n.data.id === targetNode.data.id)
  ...
  function update(source: PointNode) {
    ...
    layoutRoot
      .descendants()
      .forEach((d) => newLayoutMap.set(d.data.id, d as PointNode));
    ...
    const transition = d3
      .select(gEl)
      .transition()
      .duration(transitionDuration) as any;

    // --- NODES ---
    const node = g.selectAll('g.node').data(nodes, (d: PointNode) => d.data.id);
    ...
    const nodeEnter = node
      ...
      .on('dblclick', (event: any, d: PointNode) => {
      ...
      .on('mouseover', (_: any, d: PointNode) => {
    ...
    nodeUpdate.select('rect').each(function (
      this: SVGRectElement,
      d: PointNode
    ) {
      ...
      const nodeGroup = d3.select(rectNode.parentNode as SVGGElement);
    ...
    node
      ...
      .attr(
        'transform',
        () => `translate(${layoutSource.y ?? 0},${layoutSource.x ?? 0})`
      )
      .attr('opacity', 0);
