...
-->
<script lang="ts">
  import { TTSController } from '$lib/modules/tts';
  import { SlashMenuController } from '$lib/modules/editor';
  ...
  import WorkspaceView from '$lib/core/ui/layout/WorkspaceView.svelte';

  import { uiState, cycleColorMode } from '$lib/core/ui/uiStore.svelte';
  import {
    reviewState,
    startReview,
  ...
  import { toggle as openCommandBar } from '$lib/modules/command-bar/ui/commandBarStore.svelte';
  import { i18n } from '$lib/utils/i18n.svelte';

  function closeModal() {
    uiState.modal.show = false;
  }

  // --- Props ---
  const { onShowWelcome } = $props<{ onShowWelcome: () => void }>();
...
{#if reviewState.isReviewing && reviewState.isUiVisible}
  {#await import('$lib/modules/study/ui/ReviewController.svelte') then { default: ReviewController }}
    <ReviewController />
  {:catch error}
...
{:else}
  <div class="main-app-container">
    <AppHeader show={true} {onShowWelcome}>
      <div class="header-actions">
        ...
        <Button
          ...
          title={i18n.t(
            `visualization.color_mode.${uiState.colorMode.replace('-', '_')}`
          )}
        >
