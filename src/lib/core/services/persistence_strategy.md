...
-   **Storage**: Stores incremental *updates* (binary blobs) in IndexedDB.
...
-   A Document may *contain* references to Cards (e.g., if we embed cards in the doc), but the Card data itself lives in Dexie.
