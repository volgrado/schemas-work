...
export const dataService = {
  ...
  async deleteNode(id: string): Promise<void> {
    try {
      ...
      if (!targetNode) {
        console.warn(`[DataService] Attempted to delete non-existent node: ${id}`);
        return;
      }
      ...
      console.log(`[DataService] Deleting node ${id} and ${descendants.length} descendants.`);
      ...
      console.log(`[DataService] Successfully deleted node ${id} and cleaned up data.`);

    } catch (error) {
      errorService.reportError(error, { operation: 'dataService.deleteNode', id });
